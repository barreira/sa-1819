...
def preprocess(df):
    ...
    for idx, month in enumerate(df['Month']):
        df['Month'][idx] = month.split('-')[-1]

    df['Month'] = [months[item] for item in df['Month']]

    scaler = MinMaxScaler()
    df[['Advertising', 'Sales']] = scaler.fit_transform(df[['Advertising', 'Sales']])
    return df
...
def lstm():
    ...
    print("df", df.shape)
    janela = 12
    x_train, y_train, x_test, y_test = train_test_split(df[::-1], janela)  # o df[::-1] é o df por ordem inversa
